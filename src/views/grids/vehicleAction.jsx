...
const VehicleAction = ({ params, rowID, setRowId, vehicles, setVehicles }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const res = await updateVehicles(params.id, params.row);
    console.log(res);
    if (res == "success") {
      setSuccess(true);
      setRowId(null);
      toast.success("Vehicle Updated succesfuly!", {
  ...
  const handleDelete = async () => {
    const res = await deleteVehicle(params.id);
    if (res == "Vehicle deleted Successfully") {
      setRowId(null);
      setVehicles(vehicles.filter((row) => row._id !== params.id));
      toast.success("Vehicle Deleted succesfuly!", {
        position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
